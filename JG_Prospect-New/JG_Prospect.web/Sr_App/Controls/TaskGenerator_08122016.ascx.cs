    ...
    public partial class TaskGenerator : System.Web.UI.UserControl
    {
        ...
        private void SaveAssignedTaskUsers()
        {
            ...
            if (hdnTaskId.Value != "0")
            {
                ...
                // insert seleected users as assigned.
                // send email to selected users.
                if (sbUsersId.Length > 0)
                {
                    bool isSuccessful = TaskGeneratorBLL.Instance.SaveTaskAssignedUsers(Convert.ToUInt64(hdnTaskId.Value), sbUsersId.ToString().Substring(0, sbUsersId.ToString().Length - 1));
                    if (isSuccessful)
                    {
                        SendEmailToAssignedUsers(sbUsersId.ToString().Substring(0, sbUsersId.ToString().Length - 1));
                    }
                }
                ...
                else
                {
                    ...
                    SendEmailToAssignedUsers(strUserIDs.Substring(0, strUserIDs.Length - 1));
                }
            }
