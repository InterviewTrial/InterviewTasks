...
$serverName = "jgdbserver001.cdgdaha6zllk.us-west-2.rds.amazonaws.com"; //serverName\instanceName
    ...
    $connectionInfo = array( "Database"=>"JGBS_Interview", "UID"=>"interviewuser", "PWD"=>"JGInt@021*");
        ...
        <ul>
            ...
            <div class="content_panel col-md-12">
				...
				{ ?> 
					...
					$sql_paid ="SELECT sum(TotalPaid) as paid FROM tblTransactionDetails where tblSEId = '".$row['EstimateId']."'";
	
							$query_paid = sqlsrv_query($conn, $sql_paid);
							$row_paid = sqlsrv_fetch_array($query_paid);
							$paid=$row_paid['paid'];
							
							$sql_total ="SELECT Totalamt as total FROM tblTransactionDetails where tblSEId = '".$row['EstimateId']."'";
				...
				<?php $i++; } ?>
					...
					$sql_invoice = "Select * from tblCustomersDocument where CustomerId='".$customerID."' and  ProductTypeId='".$row['ProductTypeIdFrom']."'";
					$qry_docs = sqlsrv_query($conn, $sql_invoice);
					$row_docs = sqlsrv_fetch_array($qry_docs);
					...
					if($row_path['FileName']=='')
					{
						if($row_docs['TempName']=='')
						{
							echo $quote_num;		
						}
						else { ?>
							<a href="CustomerDocs/Pdfs/<?php echo $row_docs['TempName']; ?>" target="_blank" style="color:#000000;" ><?php echo $quote_num; ?> </a>
						<?php }
